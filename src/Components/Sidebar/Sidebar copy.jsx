...
export default function Sidebar({
  ...
  useEffect(() => {
    ...
    const observer = new MutationObserver(() => {
      ...
      if (!hasLoader) window.location.reload();
    });

    observer.observe(loaderRef.current, { childList: true, subtree: true });

    return () => observer.disconnect();
  }, [isLoading]);

  const changeTheme = () => {
    const next = currentTheme === "light" ? "dark" : currentTheme === "dark" ? "green" : "light";
  ...
  return (
    <div>
      ...
      <div className={`${isSideBarHidden ? "w-0 min-w-0" : "w-[250px] min-w-[250px] p-[20px]"} h-screen flex flex-col justify-between sidebar transition-all duration-500 ease-in-out overflow-hidden`}>
        <div>
          <p className="text-3xl mb-[40px]">{pageName}</p>

          <div className="flex flex-col gap-[10px] sidebar_item">
            {tabs.map((item, index) => (
              <Link
                to={item.link}
                className={`${path === item.link ? "active" : ""} tab_item rounded-lg group flex gap-[12px] items-center p-[10px_20px] peer`}
                key={index}
              >
                {item.image}
                <p className="whitespace-nowrap text-[18px] group-hover:text-accent">
                  {item.name}
                </p>
              </Link>
            ))}
        ...
        <div className="flex flex-col gap-[10px]">
          {userInfo ? (
            <p>Осталось дней: {userInfo.remaining_days}</p>
          ) : (
            <Loader isFull={false} />
          )}

          <button onClick={changeTheme} className="mt-[10px] p-[10px] bg-[var(--sidebar-accent)] text-white rounded-lg">
            Сменить тему
          </button>
        </div>
      </div>
